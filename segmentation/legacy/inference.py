...
def save_images(image, mask, output_path, image_file, palette):
    ...
    imgcont = np.concatenate([np.array(image)[:,:,0],mask*255], axis=1)
    cv2.imwrite(output_path+'/'+image_file+'.png', np.array(imgcont, dtype='uint8'))
    #imgcont.save(os.path.join(output_path, image_file+'.png'))
...
def main():
    ...
    assert dataset_type in ['VOC', 'COCO', 'CityScapes', 'ADE20K',"VOCself","VOCselfE", "VOCSHD"]
...
def parse_arguments():
    ...
    parser.add_argument('-mo', '--mode', default='', type=str,
