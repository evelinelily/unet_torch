...
class VOCDataset(Dataset):
    ...
    def _set_files(self):
        if "train" in self.split:
            ...
            self.files = [f for f in glob(self.root + '/20211224_png/train/' + '*.png') if
                          not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/20220105/train/' + '*.png') if not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/hard/' + '*.png') if not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/20220525_png/1/' + '*.png') if not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/20220525_png/1/' + '*.png') if not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/20220530/' + '*.png') if not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/pian/' + '*.png') if not f.endswith('-mask.png')]

            np.random.shuffle(self.files)

        else:
            self.files = [f for f in glob(self.root + '/20211224_png/val/' + '*.png') if
                          not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/20220525_png/1/' + '*.png') if not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/20220530/' + '*.png') if not f.endswith('-mask.png')] + \
                         [f for f in glob(self.root + '/20220105/val/' + '*.png') if not f.endswith('-mask.png')]
        print("load {} 数量: {}".format(self.split, len(self.files)))

    def _load_data(self, index):
        image_id = self.files[index]
        image_path = image_id
        label_path = image_id[:-4] + "-mask.png"
...
class VOCself(BaseDataLoader):
    def __init__(self,
        ...
        if split in ["train", "trainval", "val", "test"]:
            self.dataset = SolarDataset(**kwargs)
...
class SolarDataset(VOCDataset):

    def _set_files(self):
        print(5555555555555)
        suffix_list = ['.png', '.jpg']
        def check_file(im_path):
            ann = None
        ...
        if isinstance(self.root, list):
            files_all = []
            for tmp_root in self.root:
                files_all += [str(f) for suffix in suffix_list for f in Path(tmp_root).glob("**/*" + suffix)]
        else:
            files_all = [str(f) for suffix in suffix_list for f in Path(self.root).glob("**/*" + suffix)]
        files_all = [i for i in files_all if '-mask' not in i]
        assert len(files_all) > 0, self.root
        train_path, val_path = train_test_split(files_all, test_size=0.2, random_state=42)
        used_files = train_path if "train" in self.split else val_path
        self.files, self.mask_files = [], []
        for im_path in used_files:
            ann = check_file(im_path)
            if ann:
                self.files.append(im_path)
                self.mask_files.append(ann)
        print("########################")
        print("load {} 数量: {}".format(self.split, len(self.files)))
